...
from pretrained.unimodal.ecg.data_loader import TorchDataset
...
def get_args():
    ...
    parser.add_argument('--ch_names', default=['ECG'], choices=['EEG_C4', 'EEG_C3',
                                                                'EOG_Left', 'EOG_Right',
                                                                'ECG', 'EMG_Chin', 'Airflow'])
    parser.add_argument('--event_names', default=['Obstructive Apnea', 'Central Apnea', 'Mixed Apnea'])
    ...
    parser.add_argument('--ckpt_name', default='ecg', type=str)
...
class Trainer(object):
    ...
    def train(self):
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=self.args.train_batch_size, drop_last=True,
                                      shuffle=True)
        val_dataset = TorchDataset(paths=self.val_paths, ch_names=[self.args.ch_names[0]], undersampling=True,
                                   event_names=self.args.event_names, sfreq=self.args.sfreq)
