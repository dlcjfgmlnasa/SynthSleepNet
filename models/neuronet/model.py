...
class MaskedAutoEncoderViT(nn.Module):
    def __init__(self, input_size: int, num_patches: int,
        ...
        # MAE Encoder
        self.pos_embed = nn.Parameter(torch.randn(1, self.num_patches + 1, encoder_embed_dim), requires_grad=True)
        ...
        self.decoder_pos_embed = nn.Parameter(torch.randn(1, self.num_patches, decoder_embed_dim), requires_grad=True)
...
class NeuroNetEncoder(nn.Module):
    def __init__(self, fs: int, second: int, time_window: int, time_step: float,
        ...
        # "cls_token" and "pos_embed" parameter gradient is [False]
...
if __name__ == '__main__':
    ...
    oo = m0.forward(x0, mask_ratio=0.5)[0]
    print(oo)
